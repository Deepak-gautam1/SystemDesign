...
export function TheoryDetail({ topic, category, onBack, prev, next, onNavigate }: TheoryDetailProps) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      {/* Split layout — prose left, code/diagram right (mirrors ProblemDetail's proportions) */}
        ...
        {topic.code && (
          <div className="flex-1 min-w-0 flex flex-col bg-slate-950 overflow-hidden">
            <div className="shrink-0 flex items-center px-4 py-2 bg-slate-900 border-b border-slate-800">
              <span className="font-mono text-[11px] text-slate-400">{topic.codeLabel ?? "example.cpp"}</span>
            </div>
            <div className="flex-1 overflow-auto p-4">
              <CppCodeBlock code={topic.code} />
            </div>
          </div>
        )}
      </div>
