...
import {
  ArrowLeft, BookOpen, Code2, Pencil, Eye, CheckCircle2,
...
function ConceptPill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-[11px] font-medium">
      <Code2 size={9} /> {label}
    </span>
  );
}
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 border-b border-slate-700 shrink-0">
        ...
        <span className="text-[11px] font-mono text-slate-300 font-semibold">AI Code Review</span>
        ...
        <div ref={ref} className="flex-1 overflow-y-auto p-4 text-sm">
          {loading && !result && (
            <div className="flex items-center gap-2 text-slate-400 text-xs">
              <Loader2 size={12} className="animate-spin" /> Analysing your code…
            </div>
          )}
          {error && (
...
export function ProblemDetail({ problem, onBack }: ProblemDetailProps) {
  ...
  // Simple markdown description renderer
      ...
      <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-hidden">
        ...
        <div className="max-h-[35vh] md:max-h-none md:w-[40%] shrink-0 overflow-y-auto scrollbar-thin border-b md:border-b-0 md:border-r border-border p-4 md:p-5">
          ...
          <div className="space-y-1 text-sm">
            ...
            {problem.description.includes("|") && renderTable(problem.description)}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col bg-slate-950 overflow-hidden">
              ...
              {panelMode==="practice" && (
                <>
                  ...
                  <button
                    ...
                    title={!hasUserCode ? "Write some code first" : "Evaluate with AI"}
                  >
                    {evalLoading
                      ? <><Loader2 size={11} className="animate-spin" /> Evaluating…</>
                      : <><Sparkles size={11} /> Evaluate Code</>}
                  </button>
                </>
              )}
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            <div className={cn("overflow-hidden", showEval ? "flex-1" : "flex-1")}>
              {panelMode === "learn" ? (
                <div className="h-full overflow-auto p-4"><CppCodeBlock code={problem.code} /></div>
              ) : showSolution ? (
                <div className="h-full overflow-auto p-4">
                  ...
                  <CppCodeBlock code={problem.code} />
                </div>
              ) : (
                <textarea
                  value={practiceCode}
